...
function FadeIn({ children, className = '', delay = 0 }: { children: React.ReactNode; className?: string; delay?: number }) {
  ...
  return (
    <div ref={ref} style={{ transitionDelay: `${delay}ms` }} className={`transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'} ${className}`}>
      {children}
    </div>
  );
}
  ...
  if (status === 'success') {
    return (
      <div className="flex items-center gap-2">
        ...
        <p className={`font-bold text-sm ${dark ? 'text-white' : 'text-[#272F4F]'}`}>Got it. Kanth or Shaku will reply within 24 hours.</p>
      </div>
    );
  }
...
export default function Home() {
  ...
  return (
    <main style={{ fontFamily: "'DM Sans', sans-serif" }} className="bg-white text-gray-900 overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <a href={TYPEFORM_URL} target="_blank" rel="noopener noreferrer" className="bg-[#C84739] hover:bg-[#A63A2F] text-white text-sm font-black px-5 py-2.5 rounded-xl transition-all hover:scale-105 shadow-sm">
              Start Your Audit →
            </a>
          </div>
      ...
      <section className="py-28 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          <FadeIn><div className="text-center mb-16"><div className="text-sm font-black tracking-[0.2em] text-[#36488F] mb-5">THE TRANSFORMATION</div><h2 className="text-5xl md:text-6xl font-black text-[#272F4F] leading-tight">From Effort to Momentum</h2><p className="text-gray-400 mt-5 text-xl">In 90 days, the feeling of work changes.</p></div></FadeIn>
          <div className="grid md:grid-cols-3 gap-6 mb-16">{[{ title: 'The Fog Clears', desc: 'You stop asking "What should I do?" and start executing the obvious.', num: '01' }, { title: 'The Floor Rises', desc: 'Your bad days become more productive than your old good days.', num: '02' }, { title: 'The Baseline Stacks', desc: 'Your income and health finally start trending in the same direction.', num: '03' }].map((item, i) => (<FadeIn key={item.title} delay={i * 100}><div className="bg-[#F4F5F8] rounded-2xl p-10 border border-gray-100 hover:shadow-xl transition-all hover:-translate-y-1 h-full"><div className="text-sm font-black text-[#C84739] tracking-widest mb-5">{item.num}</div><h3 className="font-black text-[#272F4F] text-3xl mb-5">{item.title}</h3><p className="text-gray-500 text-xl leading-relaxed">{item.desc}</p></div></FadeIn>))}</div>
          <FadeIn><div className="text-center"><p className="text-3xl md:text-4xl font-black italic text-[#272F4F]">Stop hunting for breakthroughs.<br />Start trusting the output.</p></div></FadeIn>
        </div>
      </section>
      ...
      <section className="py-20 bg-[#272F4F] relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-6 text-center">
          <FadeIn><div className="text-xs font-black tracking-[0.2em] text-[#00C9A2] mb-3">GOT A QUESTION?</div><h2 className="text-3xl md:text-4xl font-black text-white leading-tight mb-3">Not sure if this is for you?</h2><p className="text-blue-200/60 text-base md:text-lg mb-6 leading-relaxed">Drop your email and question. Kanth or Shaku will personally reply within 24 hours — no automation, no assistant.</p><div className="flex justify-center mb-4"><EmailCapture dark source="inline-section" inline={true} /></div><p className="text-blue-300/30 text-xs">No spam. No list. Just a real reply from a real person.</p></FadeIn>
        </div>
      </section>
      ...
      <section className="py-36 bg-[#272F4F] relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto px-6 text-center">
          <FadeIn><p className="text-blue-200/50 text-xl mb-3">Stop working on yourself.</p><h2 className="text-5xl md:text-7xl font-black text-white leading-[1.05] mb-12">Start building<br />the system that<br /><span className="text-[#C84739]">works for you.</span></h2><a href={TYPEFORM_URL} target="_blank" rel="noopener noreferrer" className="inline-block bg-[#C84739] hover:bg-[#A63A2F] text-white font-black text-xl md:text-2xl px-14 py-7 rounded-2xl transition-all duration-200 shadow-2xl shadow-red-900/50 hover:scale-105">Start Your Audit →</a><p className="text-blue-300/30 text-sm mt-6">Free to start · 8 minutes · Limited spots available</p></FadeIn>
        </div>
      </section>
